...
///
/// Mirrors `PremiumLimits` on Android; the numbers have to match, or the same
...
enum PremiumLimits {
    ...
    /// account is the person this is for, and they are also the person for
    /// whom per-account tax treatment is worth money.
    static let freeAccountLimit = 2

    /// Whether another account may be created right now.
    static func canCreateAccount(currentCount: Int, isPremium: Bool) -> Bool {
        isPremium || currentCount < freeAccountLimit
    }
}
